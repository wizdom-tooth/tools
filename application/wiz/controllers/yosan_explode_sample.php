...
class Yosan_Explode_Sample extends CI_Controller_With_Auth {
	...
	public function index()
	{
		...
		$month_count = $this->input->post('_month_count');
		if ($month_count === FALSE)
		{
			$month_count = '1000';
		}
		$weekday_weights = array(
			'Mon' => $this->input->post('_weekday_weight_mon'),
			'Tue' => $this->input->post('_weekday_weight_tue'),
			'Wed' => $this->input->post('_weekday_weight_wed'),
			'Thu' => $this->input->post('_weekday_weight_thu'),
			'Fri' => $this->input->post('_weekday_weight_fri'),
			'Sat' => $this->input->post('_weekday_weight_sat'),
			'Sun' => $this->input->post('_weekday_weight_sun'),
		);
		foreach ($weekday_weights as $key => $val)
		{
			if ($val === FALSE)
			{
				$weekday_weights[$key] = '1';
			} 
		}	
		...
		foreach ($w_infos as $w_info)
		{
			...
			do {
				$yosan_week_infos[$wiz_week_id][$target_date_weekday]['date'] = $target_date;
		...
		// sample
		$month_mother_count = array_sum($yosan_week_weights);
		foreach ($yosan_week_weights as $wiz_week_id => $month_child_count)
		{
			$week_count = $month_count * $month_child_count / $month_mother_count;
			$week_mother_count = '0';
			// 分母の計算
...
//var_dump($yosan_week_infos);
		...
		$this->ag_auth->view('contents/yosan_explode_sample/index', $data);
	}
}
